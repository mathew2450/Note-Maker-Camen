...
public class MainPanel {
   ...
   /* public void createCalendarPage(){
    	JLabel contentPane = new JLabel();
        ImageIcon icon = new ImageIcon("camenLogo.png");
        contentPane.setIcon(icon);
        contentPane.setHorizontalAlignment(0);
        contentPane.setOpaque(true);
        calendarPanel.setLayout(new BorderLayout());
        calendarPanel.add(contentPane, BorderLayout.PAGE_START);
        calendarPanel.setBackground(Color.white);
        calendarPanel.setOpaque(true);
    	calendarPanel.add(calendar, BorderLayout.CENTER);
    	calendarPanel.setSize(1000,1000);
	...
	public class Login extends AbstractAction{
		public void actionPerformed(ActionEvent e) {
	        ...
	        if (BCrypt.checkpw(inputs, origPass)) {
	            ...
	            calendar = new NewButtonCalendar();
	            if(usrLvl == 1){
	            	tabbedPane.add("Calendar" + " (" + NewButtonCalendar.numLate + ")", calendar);
	...
	public class Logout extends AbstractAction{
		public void actionPerformed(ActionEvent e) {
			if(JOptionPane.showConfirmDialog(frame, 
                    "Are you sure to log out?", "Log Out", 
                    JOptionPane.YES_NO_OPTION,
                    JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION){
	...
	private static String getPassHash(String username, String hash){
		String sql;
		   try{
		      ...
		      usrLvl = rs.getInt("Employee_Level");
		      rs.close();
		      return hash;
